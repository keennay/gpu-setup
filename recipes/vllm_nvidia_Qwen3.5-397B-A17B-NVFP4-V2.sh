...
EXTRA_ARGS='--dtype bfloat16 --linear-backend cutlass --kv-cache-dtype fp8_e4m3 --attention-backend FLASHINFER --attention-config {"disable_flashinfer_q_quantization":true} --mamba-cache-dtype auto --mamba-ssm-cache-dtype float32 --enable-prefix-caching --mamba-cache-mode align --moe-backend emulation'
